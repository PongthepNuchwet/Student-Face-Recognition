...
class Frame1(tk.Frame):
    def __init__(self, windows, container):
        ...
        self.labelTitle = tk.Label(
            self.fbottom, text=' Web Came', compound=tk.LEFT, image=self.cameraIcon)
        ...
        self.btnStart = tk.Button(
            self.menu, text=" Start", image=playIcon, command=lambda: container.Start())
        ...
        self.btnStop = tk.Button(
            self.menu, text=" Stop", image=stopIcon, command=lambda: container.Stop())
...
class Frame2(tk.Frame):
    def __init__(self, windows, container):
        ...
        self.labelTitle = tk.Label(
            self.fbottom, text=' Attendance', compound=tk.LEFT, image=self.cardIcon)
        self.labelTitle.configure(bg='#40bcff', fg="white", font=('mitr', 12))
        ...
        yScroreBar = ttk.Scrollbar(
            self, orient='vertical', command=mycanvas.yview)
    ...
    def creatwidget(self, img, name, id, clas, date, time):
        ...
        self.widget = tk.Label(self.myFrame, image=image, compound=tk.LEFT,
                               text=f'   Name : {name}\n   Studen id : {id} Class : {clas}\n   Date : {date} Time : {time}')

        self.widget.configure(font=('kanit', 12),
                              justify='left', bg='#d1d1d1', fg='#000000')
