...
import os
from tkinter import Label, messagebox as ms
    ...
    def Create_Frame(self,data):
        ...
        if len(data) != 0 :
            for data in data :
                ...
                tk.Label(row,font=('mitr',12),text=f"{self.count}. Student ID : {data['id']}\tName : {data['name']}\tClass : {data['classs']}",bg='#40bcff').pack(side='left')
                btn = tk.Button(row,text=' Remove',image=self.deleteIcon,command= lambda : self.Remove(data['id']))
                btn.image = self.deleteIcon
                btn.config(relief="flat", bg="#4f4f4f", fg="white", font=('mitr', 12),compound=tk.LEFT)
                btn.pack(side='right')
                row.pack(fill='x',pady=5)
                self.count += 1
        else:
            row =  tk.Frame(self.Frame)
            label = tk.Label(row,text="ไม่พบข้อมูล",font=('kanit',16)).pack()
            row.pack(fill='x',pady=5)
