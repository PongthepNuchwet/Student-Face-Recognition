...
class Menubar(tk.Frame):
    def __init__(self, windows, container):
        ...
        self.btn1 = tk.Button(self, text=' Home', image=homeIcon,
                              command=lambda:  self.windows.Goto('startFrame'))
        self.btn1.image = homeIcon

        self.btn2 = tk.Button(self, text=' Register', image=detIcon,
                              command=lambda: self.windows.Goto('Register_view'))
        self.btn2.image = detIcon

        self.btn3 = tk.Button(self, text=' Face Recognition', image=recIcon,
                              command=lambda: self.windows.Goto('FaceRecognition_view'))
        self.btn3.image = recIcon

        self.btn4 = tk.Button(self, text=' About', image=aboutIcon,
                              command=lambda:  self.windows.Goto('About_view'))
        self.btn4.image = aboutIcon

        self.btn5 = tk.Button(self, text=' Exit',
                              image=closeIcon, command=self.windows.on_closing)
        ...
        self.LabelTime = tk.Label(
            self, text=' 17:32', compound=tk.LEFT, image=timeIcon)
        self.LabelTime.configure(bg='#dbdbdb', font=('RobotoMono', 10, 'bold'))
        self.LabelTime.image = timeIcon
        self.LabelTime.pack(side='right')

        self.Labelcalendar = tk.Label(
            self, text=' 25/7/564', compound=tk.LEFT, image=calendarIcon)
        self.Labelcalendar.configure(
            bg='#dbdbdb', font=('RobotoMono', 10, 'bold'))
