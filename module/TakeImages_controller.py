...
class TakeImages_controller:
    def __init__(self,Frame):
        ...
        if length == 0 :
            ...
            else: ms.showwarning('คำเตือน',status['text'])
        else: ms.showwarning('คำเตือน','มีรหัสนักเรียนอยู่ในฐานข้อมูลอยู่แล้ว')
    ...
    def Imwrite(self,GRAYImage,x,y,w,h):
        cv2.imwrite(f'src/Train/{self.ID}.{str(self.count)}.jpg',GRAYImage[y:y + h, x:x + w])
        self.count += 1
